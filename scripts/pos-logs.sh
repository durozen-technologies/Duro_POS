...
SERVICES=(postgres pgbouncer rustfs backend-1 backend-2 caddy)
...
Commands:
  ...
  <service>           Follow one service (postgres|pgbouncer|rustfs|backend-1|backend-2|caddy)
...
main() {
  ...
  case "${cmd}" in
    ...
    help|-h|--help)
      usage
      ;;
    postgres|pgbouncer|rustfs|backend-1|backend-2|caddy)
