...
def _needs_payment_receipt_filter(
    *,
    ...
    return payment_method is not None or payment_settled is not None or receipt_status is not None
...
async def list_shop_bills(
    ...
    *,
    ...
    return await _list_shop_bills_from_db(
        db,
        shop,
        page=page,
        page_size=page_size,
...
async def _list_shop_bills_from_db(
    ...
    *,
    ...
    if receipt_status is not None:
        ...
        payment_receipt_filters.append(cast(Receipt.receipt_status, String) == receipt_status.value)
    ...
    use_two_step = not _needs_payment_receipt_filter(
        payment_method=payment_method,
        payment_settled=payment_settled,
        receipt_status=receipt_status,
    ...
    if use_two_step:
        ...
        total_count = int(await db.scalar(select(func.count(Bill.id)).where(*bill_filters)) or 0)
        ...
        by_id = {
            bill.id: (bill, payment, receipt, username) for bill, payment, receipt, username in rows
        }
