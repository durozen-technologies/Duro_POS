"""ensure category billing item mappings

Revision ID: 0022_category_bill_maps
Revises: 0021_inv_bill_map
...
def _create_index_if_missing(index_name: str, columns: list[str]) -> None:
    bind = op.get_bind()
    if MAPPING_TABLE not in _table_names(bind) or index_name in _index_names(bind, MAPPING_TABLE):
        return
    op.create_index(index_name, MAPPING_TABLE, columns)
...
def upgrade() -> None:
    bind = op.get_bind()
    required_tables = {"inventory_items", "inventory_categories", "inventory_item_categories", "items"}
